...
function Button({
  ...
  children,
  className,
  colorClassName = "bg-green-gradient text-black font-medium",
  wrapperClassName,
  as = "button",
  ...props
}) {
  ...
  if (variant === 0) {
    return (
      ...
      >
        <span className="absolute top-0 left-0 w-full h-full bg-green-gradient  -z-10"></span>
  ...
  if (variant === 2) {
    return (
      <Component
        {...props}
        className={`text-[1em] border-2 border-main-green flex text-center justify-center font-bold  bg-green-gradient hover:bg-black text-black  transition-all duration-200 hover:bg-transparent items-center h-[2.8em] rounded-[3.75em] hover:text-white focus:border-white outline-none ${className}`}
      >
        {children}
      </Component>
    );
